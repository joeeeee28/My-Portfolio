...
async function main() {
  ...
  // Names are HTML-escaped in the rendered concept (correct — it prevents
  // injection), so compare against the unescaped document.
  const unescapedConcept = concept.html
    .replace(/&amp;/g, '&').replace(/&#39;/g, "'").replace(/&quot;/g, '"')
    .replace(/&lt;/g, '<').replace(/&gt;/g, '>');
  check('Business name appears in the concept', unescapedConcept.includes(outreachTarget.name), outreachTarget.name);
  ...
  check('Secrets are encrypted at rest', (() => {
    ...
    // Deliberately not shaped like a real credential, so the secret scan does
    // not flag this fixture. The assertion below is about encryption, not format.
    setSecret(orgId, 'e2e.test', 'e2e-fixture-plaintext-value-7890', 'E2E test key');
    ...
    return !!row && !row.cipher_blob.includes('e2e-fixture-plaintext') && row.last4 === '7890';
