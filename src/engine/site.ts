...
export function buildSiteModel(input: SiteInputs): SiteModel {
  ...
  sections.push({
    ...
    items: services.length
      ...
      : [
          { title: 'Service one', needsReview: true },
          { title: 'Service two', needsReview: true },
          { title: 'Service three', needsReview: true },
        ],
...
function deriveTagline(input: SiteInputs, theme: Theme, rand: () => number) {
  ...
  const place = input.address ? shortPlace(input.address) : null;
