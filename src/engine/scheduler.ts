 ...
 *
 * Next.js gives instrumentation.ts and route handlers separate module instances,
 * so a module-level flag set at startup would be invisible to the health check
 * and the scheduler would report "not started" while actually running.
 */
interface SchedulerState {
  started: boolean;
  tasks: Map<string, ScheduledTask>;
}
const g = globalThis as unknown as { __clientforgeScheduler?: SchedulerState };
if (!g.__clientforgeScheduler) g.__clientforgeScheduler = { started: false, tasks: new Map() };
const state = g.__clientforgeScheduler;

const startedRef = () => state.started;
const setStarted = (v: boolean) => { state.started = v; };
...
export function schedulerStatus(): { running: boolean; scheduledTasks: number; keys: string[] } {
  return { running: startedRef(), scheduledTasks: tasks.size, keys: Array.from(tasks.keys()) };
}
