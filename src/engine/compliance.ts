...
/**
 * Canonicalises a suppression value so that storage and lookup agree.
 *
 * This matters: a phone stored as "+44 117 555 7777" would never match a lookup
 * for "1175557777", silently defeating the suppression. One function, used by
 * both suppress() and isSuppressed(), makes that impossible.
 */
export function canonicalSuppressionValue(kind: string, value: string): string | null {
  switch (kind) {
    case 'phone':
...
export function isSuppressed(orgId: string, opts: { email?: string | null; phone?: string | null; domain?: string | null; businessId?: string | null }): {
  ...
  const checks: { kind: string; value: string | null }[] = [
    { kind: 'email', value: canonicalSuppressionValue('email', opts.email ?? '') },
    { kind: 'phone', value: canonicalSuppressionValue('phone', opts.phone ?? '') },
    {
      kind: 'domain',
      value: opts.domain
        ? canonicalSuppressionValue('domain', opts.domain)
        : opts.email
          ? canonicalSuppressionValue('domain', opts.email.split('@')[1] ?? '')
          : null,
    },
    { kind: 'business', value: opts.businessId ? canonicalSuppressionValue('business', opts.businessId) : null },
  ];
...
export function suppress(
  ...
  const canonical = canonicalSuppressionValue(kind, value);
  if (!canonical) {
    // An unusable identifier must not be stored as if it were a suppression —
    // that would create a false sense of coverage.
    return;
  }
  run(
    `INSERT OR REPLACE INTO suppression_list (id, org_id, kind, value, reason, source, created_at)
     VALUES (?,?,?,?,?,?,?)`,
    [id('sup'), orgId, kind, canonical, reason, source, nowIso()]
  );
  if (opts.businessId) {
...
export function handleOptOut(orgId: string, opts: { businessId: string; contactId?: string; channel: string; reason?: string }): void {
  ...
  // Suppress EVERY known identifier, not just the channel the opt-out arrived
  // on. Someone who unsubscribes by email must not then be texted, and a
  // colleague re-importing the domain must hit the same suppression.
  const reason = opts.reason ?? 'unsubscribe';
  if (business.email) suppress(orgId, 'email', business.email, reason, 'unsubscribe_link');
  if (business.phone) suppress(orgId, 'phone', business.phone, reason, 'unsubscribe_link');
  if (business.website_domain) suppress(orgId, 'domain', business.website_domain, reason, 'unsubscribe_link');
